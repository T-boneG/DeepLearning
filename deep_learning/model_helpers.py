...
HelperFunctions:
    check_inputs - check that the inputs X and Y are appropriate    ...for the given task
...
class BinaryClassificationHelper(_ModelHelper):
    ...
    def check_inputs(self, X, Y):
        ...
        # verify all elements of Y are 0 or 1 (or equivalent of a different dtype)
        Y_as_ints = np.array(Y).astype(int)
        assert np.all(np.logical_or(np.equal(Y_as_ints, 0), np.equal(Y_as_ints, 1)))
    ...
    def score(self, Y, Y_prediction):
        ...
        assert Y_prediction.size == Y.size, 'inconsistent number of samples: %d != %d' % (Y_prediction.size, Y.size)
